// Lunex lang — NTZ bytecode compiler (Go side).
//
// This file is entirely Go's responsibility.  It walks a parsed Lunex AST
// and emits a flat stream of NTZ opcodes that the Zig VM can execute.
// The opcode stream is then appended to the .nc container by format.go.
//
// Go encodes; Zig executes.  That is the entire contract.
//
// Supported AST nodes (these produce NTZ opcodes):
//   literals (null, bool, int, float, string), variables, arithmetic,
//   comparisons, bitwise ops, logical and/or, if/else/unless, while, for,
//   arrays, objects, member access, index access, break, continue, return,
//   throw, assert, typeof, void, spread.
//
// Unsupported AST nodes (fall back to Go interpreter — no NTZ emitted):
//   functions, classes, imports, async/await, generators, try/catch,
//   closures, template literals, pipelines, channels, spawn, match.
// When any unsupported node is encountered, CompileNTZ returns
// ErrNTZUnsupported and the .nc file is produced without an NTZ section.
// The Go interpreter handles execution transparently in that case.
...
// CompileNTZ walks a parsed Lunex AST and emits Zig VM-compatible bytecode.
...
// defined the compiler appends a Load+Call+Pop so the Zig VM automatically
...
// call the corresponding Zig-side global builtin.
...
// are wrapper closures that delegate to Zig-side global builtins.
// This allows programs that use standard library imports to run entirely
// under the Zig VM without falling back to the Go interpreter.
func (c *ntzC) compileAtImport(n *ast.Node) error {
        ...
        for _, m := range methods {
                ...
                // The builtin name in Zig is the module-prefixed version,
...
// moduleBuiltinName returns the Zig-side global builtin name for a module method.
