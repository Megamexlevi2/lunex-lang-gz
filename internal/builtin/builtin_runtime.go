...
// RuntimeModule returns the std.runtime built-in module.
// It takes a reference to the live interpreter so setGlobal/getGlobal can
// read and write the global environment while the program is running.
	...
	return runtime.ObjectVal(map[string]*runtime.Value{
		...
		"getGlobal": runtime.FuncVal(&runtime.Function{
			...
			Native: func(args []*runtime.Value, _ *runtime.Value) (*runtime.Value, error) {
				...
				val, _ := interp.GetGlobal(args[0].ToString())
				if val == nil {
					return runtime.Undefined, nil
				}
				return val, nil
