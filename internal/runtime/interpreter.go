...
func (interp *Interpreter) loadModule(path string) (*Value, error) {
	...
	// Prefer local files first so installed packages loaded from .lunex/cache
	// keep their real filesystem path. That preserves relative imports inside
	// the module itself (for example: ./src/node.lx).
	if localPath, ok := interp.resolveLocalFile(path); ok {
		abs, _ := filepath.Abs(localPath)
		// Deduplication check.
		interp.mu.RLock()
	...
	// Source loaders are the fallback for bundled archives and stdlib modules.
	if interp.ntlLoader != nil {
