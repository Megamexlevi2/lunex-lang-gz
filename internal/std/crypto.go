...
func bcryptDecode(s string, outLen int) ([]byte, error) {
	...
	for di < outLen {
		remaining := len(s) - si
		if remaining < 2 {
			return nil, fmt.Errorf("bcrypt: truncated input")
		}
...
func bcryptApplyRounds(st *blowfishState, ctext [24]byte) [24]byte {
...
func bcryptHash(password string, cost int) (string, error) {
	...
	ctext := bcryptApplyRounds(st, bcryptCtext)
...
func bcryptVerify(password, stored string) bool {
	...
	if stored[0] != '$' || stored[1] != '2' || stored[3] != '$' || stored[6] != '$' {
		return false
	}
