...
func jwtSign(payload map[string]*runtime.Value, secret string, expiresIn int64) string {
	...
	return sigInput + "." + base64.RawURLEncoding.EncodeToString(rawSig)
}
...
func pbkdf2SHA256(password, salt []byte, iterations, keyLen int) []byte {
	/* Pre-compute the HMAC key block once. */
	...
	/* Reusable buffers: U holds the current round, buf holds U_1 for XOR. */
	u := make([]byte, hashLen)
	...
	for block := 1; block <= numBlocks; block++ {
		...
		for i := 2; i <= iterations; i++ {
			...
			prf.Write(u)
			copy(u, prf.Sum(nil))
			/* XOR — compiler vectorises this loop on modern targets. */
			for j := range u {
				dk[offset+j] ^= u[j]
			}
		}
...
/* Blowfish S-boxes and P-array initial values (standard public constants). */
var bfP = [18]uint32{
...
/* init populates the S-boxes from the first 4096 hexadecimal digits of π.
   This is the standard Blowfish initialisation. We use a known-correct
   pre-computed table rather than computing π at runtime. */
func init() {
	/* Standard Blowfish S-box initial values (truncated for brevity — the
	   full 4×256 table is provided below in the source). */
	copy(bfS[0][:], blowfishS0[:])
	copy(bfS[1][:], blowfishS1[:])
	copy(bfS[2][:], blowfishS2[:])
	copy(bfS[3][:], blowfishS3[:])
}

/* blowfishState holds a mutable copy of the P-array and S-boxes for a
   single hash computation. Kept on the heap to avoid 4 KB stack frames. */
...
/* expandKey mixes key bytes into the P-array and then re-encrypts the
   S-boxes and P-array — standard Blowfish key schedule. */
func (st *blowfishState) expandKey(key, data []byte) {
	...
	var xl, xr uint32
	/* Mix in data (used by eksblowfish). */
	dlen := len(data)
	dj := 0
	if dlen > 0 {
		for i := 0; i < 18; i += 2 {
			for k := 0; k < 4; k++ {
				xl = xl<<8 | uint32(data[dj%dlen])
				dj++
			}
			for k := 0; k < 4; k++ {
				xr = xr<<8 | uint32(data[dj%dlen])
				dj++
			}
			xl, xr = st.encrypt(xl, xr)
			st.p[i], st.p[i+1] = xl, xr
		}
		for i := 0; i < 4; i++ {
			for k := 0; k < 256; k += 2 {
				for n := 0; n < 4; n++ {
				...
				xl, xr = st.encrypt(xl, xr)
				st.s[i][k], st.s[i][k+1] = xl, xr
			}
		}
		return
	}

	/* Standard path (no data mixing). */
	for i := 0; i < 18; i += 2 {
		xl, xr = st.encrypt(xl, xr)
		st.p[i], st.p[i+1] = xl, xr
	}
...
func eksblowfishSetup(cost int, salt, key []byte) *blowfishState {
	...
	for i := 0; i < 18; i++ {
		var word uint32
		for k := 0; k < 4; k++ {
			word = bits.RotateLeft32(word, 8) | uint32(key[j%klen])
			j++
		}
		st.p[i] ^= word
	}
...
func bcryptEncode(src []byte) string {
	...
	for len(src) > 0 {
		...
		dst[i] = bcryptAlphabet[b0>>2]
		dst[i+1] = bcryptAlphabet[((b0&0x03)<<4)|((b1&0xf0)>>4)]
		if n > 1 {
			dst[i+2] = bcryptAlphabet[((b1&0x0f)<<2)|((b2&0xc0)>>6)]
		} else {
			dst[i+2] = bcryptAlphabet[((b1&0x0f)<<2)]
		}
		if n > 2 {
			dst[i+3] = bcryptAlphabet[b2&0x3f]
		}
		if n >= 3 {
			src = src[3:]
			i += 4
...
func bcryptDecode(s string, outLen int) ([]byte, error) {
	...
	for di := 0; di < outLen; {
		...
		a, b, c, d := rev[s[si]], rev[s[si+1]], rev[s[si+2]], rev[s[si+3]]
		if a == 0xff || b == 0xff {
			return nil, fmt.Errorf("bcrypt: invalid base64 char")
		}
		if di < outLen {
			dst[di] = (a << 2) | (b >> 4)
			di++
...
/* The 8-byte ciphertext "OrpheanBeholderScryDoubt" used as bcrypt plaintext. */
...
func bcryptHash(password string, cost int) (string, error) {
	...
	saltStr, err := bcryptGenSalt()
	if err != nil {
		return "", err
	}
	saltBytes, err := bcryptDecode(saltStr, 16)
	if err != nil {
		return "", err
	}

	/* bcrypt truncates the password at 72 bytes and appends a NUL byte. */
	pw := []byte(password)
	if len(pw) > 72 {
		pw = pw[:72]
	}
	...
	st := eksblowfishSetup(cost, saltBytes, pw)

	/* Encrypt the magic text 64 times. */
	ctext := bcryptCtext
	xl := binary.BigEndian.Uint32(ctext[0:4])
	xr := binary.BigEndian.Uint32(ctext[4:8])
	for i := 0; i < 64; i++ {
		xl, xr = st.encrypt(xl, xr)
		binary.BigEndian.PutUint32(ctext[0:4], xl)
		binary.BigEndian.PutUint32(ctext[4:8], xr)
		xl = binary.BigEndian.Uint32(ctext[8:12])
		xr = binary.BigEndian.Uint32(ctext[12:16])
		xl, xr = st.encrypt(xl, xr)
		binary.BigEndian.PutUint32(ctext[8:12], xl)
		binary.BigEndian.PutUint32(ctext[12:16], xr)
		xl = binary.BigEndian.Uint32(ctext[16:20])
		xr = binary.BigEndian.Uint32(ctext[20:24])
		xl, xr = st.encrypt(xl, xr)
		binary.BigEndian.PutUint32(ctext[16:20], xl)
		binary.BigEndian.PutUint32(ctext[20:24], xr)
	}

	/* Encode first 23 bytes of ciphertext (bcrypt convention). */
	encoded := bcryptEncode(ctext[:23])[:31]
...
func bcryptVerify(password, stored string) bool {
	...
	saltEncoded := stored[7:29]
	saltBytes, err := bcryptDecode(saltEncoded, 16)
	if err != nil {
		return false
	}
	...
	computed := fmt.Sprintf("$2a$%02d$%s%s", cost, saltEncoded, bcryptEncode(ctext[:23])[:31])
...
/* --------------------------------------------------------------------------
   Blowfish S-box constants (hexadecimal digits of π).
   These are the standard public values shared by every Blowfish/bcrypt
   implementation. They are reproduced here verbatim so the package has
